...
//AuthenticationManager가 실질적으로 사용자의 정보를 조회할 떄 메소드를 호출하는 클래스
	...
	log.info("이거 오나?{}",user);
		...
		if(user== null) {
			throw new UsernameNotFoundException("야야 이거 아니야 다시 입력해");
		}
		
		return CustomUserDetails.builder().username(user.getMemberId())
										  .password(user.getMemberPwd()) //여기에는 암호화된 비밀번호가 들어와 있음
